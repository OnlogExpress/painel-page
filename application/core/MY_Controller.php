...
class MY_Controller extends \CI_Controller {
	...
	protected function _detect_method() {
		...
		return in_array($method, $this->allowed_http_methods) && method_exists($this, '_parse_' . $method) ? $method : $this->no_requets($method);
	}
	...
	protected function autentictionToken($token) {

		$data = $this->apiModel->find('company', "token_api = '{$token}' LIMIT 1")->row_array();

		if (empty($data) || empty($token)) {
			return false;
		}
		$this->session->set_userdata($data);
		$this->setCompanyId($this->session->userdata('id'));
		$this->setTablePriceId($this->session->userdata('table_prace_id'));
		$this->setCustPriceFreight($this->session->userdata('freight_cost'));
		return true;
	}

	protected function autentictionTokenMotorist($token) {
		$data = $this->apiModel->find('motorists', "token = '{$token}' LIMIT 1")->row_array();
		if (empty($data) || empty($token)) {
			return false;
		}
		return true;
	...
	private function no_requets($method){
		return show_404('teste');
	}
}
